...
- **Tokens, not values.** Every visual knob (`tone`, `gap`, `pad`, `radius`) is a semantic token name the host's renderer maps to its own design ([invariant 6](../ARCHITECTURE.md#invariants)).
  Tones are `ok | warn | bad | subtle | neutral | …` - an open set the theme defines.
...
| Block     | What it is                                                                                     |
| --------- | ---------------------------------------------------------------------------------------------- |
| `Box`     | The plain container; `pad` token                                                               |
| `Stack`   | Flow layout; `direction="horizontal"` for a row, `gap` token                                   |
| `Grid`    | Column grid; `cols` (default 12) is the design grid, children divide it equally, `gap` token   |
| `Divider` | A hairline rule                                                                                |
| `Card`    | A themed surface - padding, radius, border, and background from the theme; `gap`, `pad` tokens |

## Content

| Block      | What it is                                                              |
| ---------- | ----------------------------------------------------------------------- |
| `Text`     | Body text; `tone`, `weight="bold"`, `size="xl"`                         |
| `Heading`  | `level={1..6}`, default 2                                               |
| `Markdown` | Sanitized prose; no HTML passthrough                                    |
| `Image`    | `src`, **`alt` required**, `aspect`                                     |
...
| Block          | What it is                                                                                 |
| -------------- | ------------------------------------------------------------------------------------------ |
| `Button`       | `tone="primary"` for the filled variant; actions via `on:event`                            |
...
| Block              | What it is                                                                                                                           |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| `Tabs`             | `items` = labels, `active` = default tab (label or index), one child panel per item, `variant="underline"`; bind to drive from state |
...
All of these are rich blocks: each carries a normative `decomposeTo` recipe, so a renderer that cannot draw one renders the primitive expansion instead ([invariant 8](../ARCHITECTURE.md#invariants)).
...
| `Chart`     | One chart; `type = line \| area \| bar \| donut \| radar \| gauge \| scatter \| heatmap \| sankey \| sparkline`, data in prop-arrays, **`alt` required** | Its `alt` text                                            |
...
Recurring whole-interface shapes (`Plan`, `Incident`) are [host macros](proposal.md#44-host-macros) that expand to these blocks before validation, not vocabulary.

## Unknown tags

A tag not in the registry is a warning by default - the reference renderer draws a dashed debug box so the content still shows - and an error when the manifest sets `strict: true`.
